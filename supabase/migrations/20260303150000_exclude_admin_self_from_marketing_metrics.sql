...
DECLARE
  ...
  v_result jsonb;
  v_excluded_user_id uuid := auth.uid();
BEGIN
  ...
  WITH excluded_sessions AS (
    ...
    WHERE user_id = v_excluded_user_id
  ...
  pageviews_window AS (
    ...
    WHERE created_at >= v_since
      AND session_id NOT IN (SELECT session_id FROM excluded_sessions)
  ),
  sessions_window AS (
  ...
  first_seen_all AS (
    ...
    FROM public.marketing_pageviews
    WHERE session_id NOT IN (SELECT session_id FROM excluded_sessions)
    GROUP BY session_id
  ),
  sessions_labeled AS (
    SELECT
  ...
  signups_window AS (
    ...
    WHERE criado_em >= v_since
      AND id IS DISTINCT FROM v_excluded_user_id
  ),
  top_search_terms AS (
    SELECT
  ...
  daily AS (
    ...
    LEFT JOIN public.usuarios u
      ...
      AND u.id IS DISTINCT FROM v_excluded_user_id
    GROUP BY day::date
